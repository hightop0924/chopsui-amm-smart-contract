module defi::chopsui {
    ...
    use defi::chopsui_library::{quote, sqrt, get_amount_out, get_amount_in, compare, is_overflow_mul, overflow_add};
    use defi::uq64x64;
    // use std::debug;
...
module defi::chopsui_tests {
    ...
    fun test_add_remove_lp_basic() {
        ...
        next_tx(&mut scenario, owner);
        {
            ...
            chopsui::init_for_testing(ctx(test));
        ...
        next_tx(&mut scenario, two);
        {
            ...
            chopsui::create_pair<TestCoin1, TestCoin2>(&mut lps, ctx(test));
        ...
        next_tx(&mut scenario, one);
        {
            ...
            let lp_coins = chopsui::add_liquidity<TestCoin1, TestCoin2>(
                &mut lps,
                &clock,
        ...
        next_tx(&mut scenario, one);
        {
            ...
            let lps = test::take_shared<LiquidityPools>(test);
            let clock = test::take_shared<Clock>(test);
            let (coin_x, coin_y) = chopsui::remove_liquidity<TestCoin1, TestCoin2>(
                &mut lps,
                &clock,
    ...
    fun test_swap_lp_basic() {
        ...
        next_tx(&mut scenario, one);
        {
            ...
            let lps = test::take_shared<LiquidityPools>(test);
            let clock = test::take_shared<Clock>(test);
            let (zero, coins_out) = chopsui::swap_coins_for_coins<TestCoin1, TestCoin2>(
                &mut lps,
                &clock,
        ...
        next_tx(&mut scenario, one);
        {
            ...
            let lps = test::take_shared<LiquidityPools>(test);
            let clock = test::take_shared<Clock>(test);
            let (coins_out, zero) = chopsui::swap_coins_for_coins<TestCoin1, TestCoin2>(
                &mut lps,
                &clock,
                coin::zero(ctx(test)),
    ...
    fun test_swap_lp_multiple_exact_x_y_z() {
        ...
        next_tx(&mut scenario, owner);
        {
            ...
            let lps = test::take_shared<LiquidityPools>(test);
            chopsui::create_pair<TestCoin1, TestCoin2>(&mut lps, ctx(test));
            chopsui::create_pair<TestCoin2, TestCoin3>(&mut lps, ctx(test));
            test::return_shared(lps);
        };
        next_tx(&mut scenario, one);
        {
        ...
        next_tx(&mut scenario, one);
        {
            ...
            let lps = test::take_shared<LiquidityPools>(test);
            let clock = test::take_shared<Clock>(test);
            let lp_coins = chopsui::add_liquidity<TestCoin2, TestCoin3>(
                &mut lps,
                &clock,
        ...
        next_tx(&mut scenario, one);
        {
            ...
            let lps = test::take_shared<LiquidityPools>(test);
            let clock = test::take_shared<Clock>(test);
            chopsui::swap_exact_coins_for_coins_2_pair_entry<TestCoin1, TestCoin2, TestCoin3>(
                &mut lps,
                &clock,
                mint<TestCoin1>(1000, ctx(test)),
                1000,
    ...
    fun test_swap_lp_multiple_x_y_z_exact() {
        ...
        next_tx(&mut scenario, one);
        {
            ...
            let lps = test::take_shared<LiquidityPools>(test);
            let clock = test::take_shared<Clock>(test);
            chopsui::swap_coins_for_exact_coins_2_pair_entry<TestCoin1, TestCoin2, TestCoin3>(
                &mut lps,
                &clock,
    ...
    fun test_swap_lp_multiple_exact_z_y_x() {
        ...
        next_tx(&mut scenario, one);
        {
            ...
            let lps = test::take_shared<LiquidityPools>(test);
            let clock = test::take_shared<Clock>(test);
            chopsui::swap_exact_coins_for_coins_2_pair_entry<TestCoin3, TestCoin2, TestCoin1>(
                &mut lps,
                &clock,
    ...
    fun test_swap_lp_multiple_z_y_x_exact() {
        ...
        next_tx(&mut scenario, one);
        {
            ...
            let lps = test::take_shared<LiquidityPools>(test);
            let clock = test::take_shared<Clock>(test);
            chopsui::swap_coins_for_exact_coins_2_pair_entry<TestCoin3, TestCoin2, TestCoin1>(
                &mut lps,
                &clock,
    ...
    #[test]
    #[expected_failure(abort_code = chopsui::ERR_PAIR_ALREADY_EXIST)]
